...
Source: [https://github.com/mikewolak/epstool/blob/main/eps_sysex.md](https://github.com/mikewolak/epstool/blob/main/eps_sysex.md)

---

> ## THIS DOCUMENT DESCRIBES THE ORIGINAL EPS, NOT THE EPS-16 PLUS
>
> Despite the intro text above, it covers the Classic only. This has been
> confirmed against the scanned original — Ensoniq's *EPS External Command
> Specification*, June 12 1989, MKB2 — which is **the authority, and this is
> not**. Prefer the PDF wherever you can; this transcription is for grepping.
>
> The comparison of the two machines, and the list of where this file departs
> from the original, are in
> **[eps-classic-vs-16plus.md](eps-classic-vs-16plus.md)**.
>
> ### Traps before you use a number from this document
>
> **1. Its page numbers are a quarter of ours.** It indexes parameter pages by
> `number >> 8`, `0x00`–`0x0E`. This project and section 9 of the 16 PLUS manual
> index them by the *wire* high byte, `number >> 6`, `$00`–`$38`. So this
> document's "0x0D System" is our `$34`, its "0x0C MIDI" is our `$30`, and its
> "0x0E Edit" is our `$38`. Ensoniq states the rule outright: "high byte times
> four followed by the low byte… Free System Blocks is transmitted via MIDI
> bytes `$34 $00` not `$0D $00`." `EPS16.parameterNumber(page, item)` takes this
> document's convention; `EPS16.getParameter(a, b)` takes the wire bytes.
> Confusing the two yields a different, perfectly valid parameter number and no
> error at all.
>
> **2. It writes page numbers in hex and item numbers in decimal**, and says so
> nowhere. Ensoniq writes both in decimal: "SYSEX HIGH BYTE: 13", item 10. So
> "Wavedata Start 21" means item `$15` and "Velocity High 10" means item `$0A`.
>
> **3. It is missing COPY INSTRUMENT (`$12`), Table 7, and — the costly one —
> the line at the head of every parameter block table saying "all values in hi
> byte of word".** That line is what tells you the Classic keeps wavesample pan
> in the high byte of word 105.
>
> Its parameter numbers, ranges and value tables are otherwise faithful,
> **including several things that look like errors and are Ensoniq's own** —
> notably modulation-source ranges of 0–17 and 0–18 against a Table 2 that lists
> only 0–15. Do not "correct" those against the 16 PLUS manual without checking
> the PDF first.
