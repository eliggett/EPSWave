...
/****** eps.lib/epsSaveInstrument ******************************************
*
*   NAME   
*	epsSaveInstrument -- save an instrument to disk.
*
*   SYNOPSIS
*	error = epsSaveInstrument( eps, inst )
*
*	int epsSaveInstrument( struct EPSdesc *, edit_spec * );
*
*   FUNCTION
*	Save the instrument described in `inst' to the current directory of
*	the current storage device.
*
*   INPUTS
*	eps		- descriptor for the EPS to send the command to.
*	inst		- the instrument to be saved.
*
*   RESULT
...
*
*   NOTES
*	This command only works on the EPS 16+.
*
*   BUGS
*	Not tested.
*
...
edit_spec *inst;		/* Which instrument? */
{
	...
	MsgByte(0x03);
	MsgByte(eps->chan);
	MsgByte(CMD_SAVE_INST);	/* Request instrument parameters */
